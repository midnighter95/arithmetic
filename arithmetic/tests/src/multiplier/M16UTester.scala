...
object M16UTester extends TestSuite with ChiselUtestTester {
  def tests: Tests = Tests {
    test("M16U should pass") {
      def testcase(width: Int): Unit = {
        ...
        val aInput   = "b" + getCircuitValue(a, 16)
        val bInput   = "b" + getCircuitValue(b, 16)
        val z_expect = "b" + getCircuitValue(z, 32)
